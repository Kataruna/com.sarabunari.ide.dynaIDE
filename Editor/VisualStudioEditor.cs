...
namespace Microsoft.Unity.VisualStudio.Editor
{
	...
	public class VisualStudioEditor : IExternalCodeEditor
	{
		...
		public void OnGUI()
		{
			...
			if (installation is VisualStudioDynaIDEInstallation)
			{
				var reuseWindow = EditorPrefs.GetBool(VisualStudioDynaIDEInstallation.ReuseExistingWindowKey, false);
				var newReuseWindow = EditorGUILayout.Toggle(new GUIContent("Reuse existing DynaIDE window", "When enabled, opens files in an existing DynaIDE window if found. When disabled, always opens a new window."), reuseWindow);
				if (newReuseWindow != reuseWindow)
					EditorPrefs.SetBool(VisualStudioDynaIDEInstallation.ReuseExistingWindowKey, newReuseWindow);
